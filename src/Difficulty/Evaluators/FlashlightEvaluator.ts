...
export class FlashlightEvaluator {
  private static _MAX_OPACITY_BONUS = 0.4;
  private static _HIDDEN_BONUS = 0.2;
  private static _MIN_VELOCITY = 0.5;
  private static _SLIDER_MULTIPLIER = 1.3;
  private static _MIN_ANGLE_MULTIPLIER = 0.2;
  ...
  static evaluateDifficultyOf(current: DifficultyHitObject, hidden: boolean): number {
    ...
    for (let i = 0; i < Math.min(current.index, 10); i++) {
      ...
      if (!(currentObj.baseObject instanceof Spinner)) {
        ...
        const opacityBonus = 1 + this._MAX_OPACITY_BONUS * (1 - opacity);
     ...
     */
    if (hidden) result *= 1 + this._HIDDEN_BONUS;

    /**
     * Nerf patterns with repeated angles.
     */
    result *= this._MIN_ANGLE_MULTIPLIER
      + (1 - this._MIN_ANGLE_MULTIPLIER) / (angleRepeatCount + 1);
    ...
    if (osuCurrent.baseObject instanceof Slider) {
       ...
       */
      const max1 = Math.max(
        0, pixelTravelDistance / osuCurrent.travelTime - this._MIN_VELOCITY,
      );
    ...
    result += sliderBonus * this._SLIDER_MULTIPLIER;
